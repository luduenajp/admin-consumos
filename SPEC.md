...
- **Steps:**
  1. `POST /share-target` (no auth, in `main.py`) receives the multipart POST directly — the service worker (`frontend/public/sw.js`) deliberately does **not** intercept this request. It used to (stashing the file in Cache API), but the request body arrived empty at the SW's `event.request.formData()`.
  2. The handler tries Starlette's standard multipart parser (`request.form()`) first. If that finds no file part, it falls back to `recover_file_part()` (`backend/app/multipart_recovery.py`) — a lenient hand-rolled parser that locates the named part by splitting the raw body on the boundary delimiter directly, tolerant of malformed framing.
  3. If a file was recovered either way, it's stashed server-side (in-memory, one-shot, 5-minute TTL — `backend/app/share_target_store.py`) under a random token and the response is `303 → /nueva-transferencia?shared=1&token=<token>`. If no file was found by either parser, `303 → /nueva-transferencia` with an empty form (diagnostic `print()`s log content-type/body length/head+tail bytes in this case, to debug new client variants without needing device access).
  4. The `/nueva-transferencia` page fetches the pending file via `GET /api/share-target/pending/{token}` (`fetchPendingSharedComprobante()` in `frontend/src/api/endpoints.ts`), which consumes (pops) the entry — a second request for the same token returns `404`. `retrieveSharedFile()` (`frontend/src/utils/sharedFile.ts`, Cache API) is tried first as a defensive fallback in case some browser does deliver the file to the SW, but is not relied upon.
  5. The file is injected into `PurchaseForm` (`initialFile` prop) with `payment_method: 'transfer'`, triggering UC-053 extraction and auto-fill
  6. The user confirms and saves manually (confirm-before-save)
- **Known client-side quirks worked around here:**
  - **Chrome ~153 on Android (WebAPK installs):** a Chromium regression (commit adding `WebAppLaunchHandler.filterShareData()` / `ComponentCaller.checkContentUriPermission()`, "[M153] Verify caller URI permissions before Web Share Target in TWA") strips the shared file's URI *before* the multipart body is even constructed — the POST arrives with a well-formed envelope but zero parts (observed: exactly the closing boundary, ~75 bytes), reproducible with any source app or file type. Not fixable from this codebase; the practical workaround is installing the PWA from a browser with an older engine (e.g. Samsung Internet) instead of Chrome. Tracked upstream by Google; see squoosh#1503 for community confirmation.
  - **Samsung Internet:** delivers the file correctly over the network (bypassing the Chrome bug above), but has been observed inserting a stray NUL byte between a part's content and the closing boundary delimiter, which Starlette's/`python-multipart`'s parser rejects outright (0 parts found despite a full-size body). `recover_file_part()` handles this by splitting on the boundary substring itself instead of requiring exact `\r\n--boundary` adjacency, and stripping trailing `\r\n\x00` noise from the recovered content.
